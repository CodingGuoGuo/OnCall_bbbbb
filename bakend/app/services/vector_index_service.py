...
from app.services.document_splitter_service import SplitOptions, document_splitter_service
...
class VectorIndexService:
    ...
    def index_single_file(
        ...
        Returns:
            (分片列表, 统计信息) —— 供上传接口把本次切出的 chunk 回传给前端做临时预览

        Raises:
            ValueError: 文件不存在或切片参数非法
        ...
        try:
            ...
            # 3. 按传入参数切分（旧路径不传 options 时行为与改造前一致）
            documents, stats = document_splitter_service.split_with_options(
                content, normalized_path, options
            )
