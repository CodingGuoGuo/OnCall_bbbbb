...
class DashScopeEmbeddings(Embeddings):
    ...
    # DashScope 的 text-embedding 单次最多接受 10 条，超过会直接 400：
    #   "batch size is invalid, it should not be larger than 10"
    # 这里必须自己分批，否则只要文件切出超过 10 块就无法入库。
    EMBED_BATCH_SIZE = 10
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        Returns:
            List[List[float]]: 嵌入向量列表（顺序与入参一致）
        """
        if not texts:
            return []
        ...
        try:
            total = len(texts)
            logger.info(
                f"批量嵌入 {total} 个文档（每批 {self.EMBED_BATCH_SIZE} 条，"
                f"共 {-(-total // self.EMBED_BATCH_SIZE)} 批）"
            )
            ...
            for start in range(0, total, self.EMBED_BATCH_SIZE):
                ...
                logger.debug(f"嵌入进度 {min(start + len(batch), total)}/{total}")

            # 数量对不上就直接失败，避免向量与文档错位
            if len(embeddings) != total:
                raise RuntimeError(
                    f"嵌入数量不匹配：期望 {total} 条，实际返回 {len(embeddings)} 条"
                )
